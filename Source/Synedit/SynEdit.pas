...
// use scAll to update a statusbar when another TCustomSynEdit got the focus
  ...
  private
{$IFNDEF SYN_CLX}
    procedure WMCancelMode(var Message: TMessage); message LM_CANCELMODE;
    procedure WMCaptureChanged(var Msg: TMessage); message LM_CaptureChanged;
    procedure WMClear(var Msg: TMessage); message LM_CLEAR;
    procedure WMCopy(var Message: TMessage); message LM_COPY;
    procedure WMCut(var Message: TMessage); message LM_CUT;
    procedure WMDropFiles(var Msg: TMessage); message WM_DROPFILES;
    procedure WMEraseBkgnd(var Msg: TMessage); message WM_ERASEBKGND;
    procedure WMGetDlgCode(var Msg: TWMGetDlgCode); message WM_GETDLGCODE;
    procedure WMGetText(var Msg: TLMGetText); message 'WM_GETTEXT';
    procedure WMGetTextLength(var Msg: TLMGetTextLength); message LM_GETTEXTLENGTH;
    procedure WMHScroll(var Msg: TWMScroll); message WM_HSCROLL;
    procedure WMPaste(var Message: TMessage); message LM_PASTE;
    procedure WMSetText(var Msg: TLMSetText); message 'WM_SETTEXT';
  {$IFDEF SYN_MBCSSUPPORT}
  ...
  {$IFNDEF SYN_LAZARUS}
    procedure WMSetCursor(var Msg: TMessage); message LM_SETCURSOR;
  {$ENDIF}
  ...
  // fail if it's bigger than 64KB. It's useless to set the Caption anyway.
{
  ...
  begin
    ...
    if NewStyleControls {$IFNDEF SYN_LAZARUS}and Ctl3D{$ENDIF} and (fBorderStyle = bsSingle) then
    begin
...
{$ELSE}
        ...
        begin
          ...
          LCLIntf.ExtTextOut(DC, (fGutterWidth - fGutter.RightOffset - 2) - TextSize.cx,
...
{$IFNDEF SYN_LAZARUS}
  ...
  begin
    ...
    begin
      ...
      LCLIntf.GetCharWidth( dc, iLastChar, iLastChar, iCharWidth );
...
begin
  {$IFDEF SYN_LAZARUS}
//    LastMouseCaret:=Point(-1,-1);   //SynEdit.pas(4817,19) Error: Identifier not found "LastMouseCaret"
...
procedure TCustomSynEdit.WMGetText(var Msg: TLMGetText);
begin
  StrPLCopy(Msg.Text, Text, Msg.TextMax - 1);
  Msg.Result := StrLen(Msg.Text);
end;

procedure TCustomSynEdit.WMGetTextLength(var Msg: TLMGetTextLength);
begin
{$IFDEF SYN_COMPILER_4_UP}
...
{$IFNDEF SYN_LAZARUS}
    ...
    if bSetDrag and not (csDesigning in ComponentState) and HandleAllocated then
      DragAcceptFiles(Handle, (eoDropFiles in fOptions));  //SynEdit.pas(8175,22) Error: Identifier not found "DragAcceptFiles"
{$ENDIF}
...
begin
  ...
  if (eoDropFiles in fOptions) and not (csDesigning in ComponentState) then
    DragAcceptFiles(Handle, TRUE); //SynEdit.pas(8994,20) Error: Identifier not found "DragAcceptFiles"
...
begin
  {$IFNDEF SYN_LAZARUS}
  if (eoDropFiles in fOptions) and not (csDesigning in ComponentState) then
    DragAcceptFiles(Handle, FALSE); //SynEdit.pas(9005,20) Error: Identifier not found "DragAcceptFiles"
...
begin
  LCLIntf.InvalidateRect( Handle, @aRect, aErase );
end;
{$ENDIF}
